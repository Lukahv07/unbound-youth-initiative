...
const Header = () => {
  ...
  useEffect(() => {
    ...
    const sections = ['about', 'about-us', 'partnership', 'methodology', 'media', 'contact'];
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            <button onClick={() => scrollToSection('about-us')} className={`transition-colors ${isWhiteHeader ? 'text-foreground hover:text-primary' : 'text-white hover:text-white/80'}`}>
              About Us
            </button>
      ...
      {isMenuOpen && (
        <div className={`md:hidden border-t transition-colors duration-300 ${isWhiteHeader ? 'bg-background border-border' : 'bg-blue-600/95 border-blue-500/30'}`}>
          <div className="px-2 pt-2 pb-3 space-y-1">
            <button onClick={() => scrollToSection('about-us')} className={`block px-3 py-2 text-base font-medium w-full text-left transition-colors ${isWhiteHeader ? 'text-foreground hover:text-primary' : 'text-white hover:text-white/80'}`}>
              About Us
            </button>
